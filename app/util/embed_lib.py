...
def to_hex(color):
    if isinstance(color, str):
        # Assuming the input is a hexadecimal color string
        color = color.strip('#')
        r = int(color[0:2], 16)
        g = int(color[2:4], 16)
        b = int(color[4:6], 16)
    elif isinstance(color, tuple) and len(color) == 3:
        # Assuming the input is an RGB tuple
        r, g, b = color
    else:
        raise ValueError("Invalid input. Expected hex color string or RGB tuple.")
...
class InvalidMojangUserEmbed(discord.Embed):
    def __init__(self, is_invalid_uuid=False, player=None):
        super().__init__()
...
class SuccessfullyUnlinkedEmbed(discord.Embed):
    def __init__(self, mention):
        super().__init__()
        self.colour = discord.Colour.from_str("#27db66")
        self.description = f"Successfully unlinked your account {mention}!"
